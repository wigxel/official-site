...
export function ArticleContext({
  ...
  useEffect(() => {
    headingRefs.current = {}
    const headings = Array.from(document.querySelectorAll('h2')).map((el, index) => ({ el, id: String(index) }))
  ...
  return (
    <aside className="top-[var(--header-height)] hidden w-1/4 shrink flex-col gap-4 md:sticky md:flex">
      ...
      <div className="relative group/root">
        <ul role="status" className="flex flex-col gap-3 pointer-events-none select-none group-hover/root:opacity-0">
            ...
            return (
              ...
              >
                <span className="flex items-center gap-1 text-sm tabular-nums">
                  <span className="inline-block w-4 border-b-2 border-foreground transition-default group-data-[active=true]:w-6 group-data-[active=true]:opacity-100 opacity-50" />
                </span>
              </li>
            )
          })}
        ...
        <ul className="flex absolute text-pretty top-0 z-20 opacity-0 transition-default group-hover/root:opacity-100 bg-background/80 max-w-[30ch] left-0 flex-col gap-3">
          {headings.map((e, index) => {
            const isActive = String(index) === activeHeadingId
