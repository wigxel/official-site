...
import "./carousel";
import { menuArea, yScroll, activateScrollRemote } from "./menu.js";
...
const animateText = (words, el, divider = " ") => {
  ...
  const playSentence = el => () => {
    [...el.children].forEach((elem, i) =>
      setTimeout(() => playWord(elem), i * 200)
    );
  };
...
function handleParallax() {
  ...
  return () => {
    elems.forEach(e => {
      ...
      if (props.inScope()) requestAnimationFrame(() => props.move());
    });
  };
}
...
function scrollTo({ x, y }, duration = 1000) {
  const cords = {
    x: window.scrollX,
    y: window.scrollY
  };
...
function makeWibble() {
  return anime({
    ...
    d: [
      {
        value:
          "M2793.969,59.132c-12.073,45.1,19.635,90.114,59.662,104.884,46.49,17.154,107.379-20.956,107.022,49.107s-3.645,85.823,35.156,84.564,141.475-61.931,134.4-6.928c-19.375,146.134,61.406,101.685,100.954,88.594.085.086.824-319.687.824-319.687S2794.354,59.1,2793.969,59.132Z"
      },
      {
        value:
          "M2793.969,59.132c-38.989,29.7-27.882,60.293,0,77.291,56.255,30.35,103.466,63.9,103.109,133.967s71.529,41.242,92.706,36.276,76-27.671,99.944,7.066c28.71,65.825,101.892,78.712,141.44,65.621.085.086.824-319.687.824-319.687S2794.354,59.1,2793.969,59.132Z"
      }
    ],
    delay: 300,
    loop: true,
...
function activateCursor() {
  ...
  const otherLinks = document.querySelectorAll(
    ".scroll-remote li, .scroll-remote .menubar"
  );
